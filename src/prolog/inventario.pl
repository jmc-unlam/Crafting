% Objetos en el inventario
inventario('mineral de plata',3).
inventario('lingote de hierro',2).
inventario('Mesa de Flechas',1).
inventario('madera',980).
inventario('carbon',8).
inventario('mineral de oro',3).
inventario('mineral de estano',4).
inventario('Punta de Flecha',20).
inventario('Mesa Generica',1).
inventario('mineral de cobre',3).
inventario('ladrillo',960).
inventario('Objeto Default', 0).
